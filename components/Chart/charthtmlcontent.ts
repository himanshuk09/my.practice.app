export let WebviewLineHtmlContent = `
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8" />
        ...
        <script>
            ...
            function toggleMarkers() {
                ...
                window.ReactNativeWebView.postMessage(
                    JSON.stringify({ action: "startLoader" })
                );
                ...
                window.ReactNativeWebView.postMessage(
                    JSON.stringify({
                        action: "tooltip",
                        values: currentSize === 0 ? true : false,
                    })
                );
                ...
                window.ReactNativeWebView.postMessage(
                    JSON.stringify({ action: "stopLoader" })
                );
            }
            ...
            function renderChart() {
                const options = {
                    ...
                    chart: {
                        ...
                        height: "285",
                        background:
                            "url('https://i.ibb.co/HdCGLJn/default-large-chart.png') no-repeat center center",
                        stacked: false,
                        ...
                        events: {
                            ...
                            beforeZoom: function (
                                ...
                                if (
                                    ...
                                    window.ReactNativeWebView.postMessage(
                                        JSON.stringify({
                                            action: "Zoom Prevented",
                                            reason: "No data points visible",
                                        })
                                    );
                                    return {
                                        xaxis: {
                    ...
                    responsive: [
                        {
                            ...
                            options: {
                                chart: {
                                    height: "95%",
                                    background:
                                        "url('https://i.ibb.co/wgS847n/default-large-chart.png') no-repeat center center",
                                },
                            },
                        },
                    ],
            ...
            function resetChartSeries() {
                chart.resetSeries();
                window.ReactNativeWebView.postMessage(
                    JSON.stringify({ action: "resetChartSeries" })
                );
            }
            ...
            function updateLocale(newLocale, yAxisTitle) {
                const xaxisTitle =
                    newLocale === "de" ? "Datum / Uhrzeit" : "Date / Time";
                const selectedLocale = locales[newLocale];

                window.ReactNativeWebView.postMessage(
                    JSON.stringify({
                        action: "updateLocale",
                        value: [newLocale, xaxisTitle],
                    })
                );
                        ...
                        y: {
                            formatter: (value) => {
                                const formatter = new Intl.NumberFormat(
                                    newLocale === "de" ? "de-DE" : "en-IN",
                                    { maximumFractionDigits: 2 }
                                );
                                return (
                                    formatter.format(value) + " " + yAxisTitle
                                );
                            },
                        },
                        x: {
                ...
                chart.updateOptions({
                    xaxis: {
                        labels: {
                            formatter: (value) => {
                                const xAxisData =
                                    chart.w.globals.initialSeries[0].data;
                                const index =
                                    chart.w.globals.labels.indexOf(value);
                                const date = new Date(value);
                                let formatOptions = {};
                                switch (type) {
                                    case "Day":
            ...
            window.customPanLeft = function () {
                const moveFactor =
                    (chart.w.globals.maxX - chart.w.globals.minX) * 0.3;

                // Calculate the new min and max X values for the pan
                const newMinX = Math.max(
                    chart.w.globals.minX - moveFactor,
                    chart.w.globals.seriesX[0][0]
                ); // Prevent going past the series minimum
                const newMaxX = Math.max(
                    chart.w.globals.maxX - moveFactor,
                    chart.w.globals.seriesX[0][0] +
            ...
            function highlightMinAndMax(chartInstance) {
                ...
                if (Array.isArray(seriesData[0])) {
                    ...
                    seriesData[0].x !== undefined &&
                    seriesData[0].y !== undefined
                ) {
                    // { x, y } format
                    const minPoint = seriesData.reduce(
                        (min, point) => {
    ...
    <body>
        ...
        <script>
            ...
            function updateChartSeries(filteredData) {
                ...
                window.ReactNativeWebView.postMessage(
                    JSON.stringify({ action: "updateChartSeries" })
                );
            }
            ...
            window.customPanLeft = function () {
                const moveFactor =
                    (chart.w.globals.maxX - chart.w.globals.minX) * 0.5;
...
`;
export const iframeAreahtlcontent = `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
